...
#define CHAR_TO_DIGIT(BASE) static_cast<T>(yych - (BASE))
...
[[gnu::always_inline]] inline ParseNumberResult<T, get_digits> lex_oct_digits_unsigned (const char* YYCURSOR) {
    ...
    only_zeros:{
        /*!local:re2c
            [0]         { goto only_zeros;                                      }
            [1-7]       { value = CHAR_TO_DIGIT('0');   goto not_only_zeros;    }
            *           { RETURN_ONLY_ZEROS()                                   }
        */
    }
    not_only_zeros:
    ...
    for (size_t i = 1; i < ce::log<8, max> + 2; i++) {
        /*!local:re2c
            [0-7]       { value = value << 3 + CHAR_TO_DIGIT('0');  continue;   }
...
[[gnu::always_inline]] inline ParseNumberResult<T, get_digits> lex_oct_digits_signed (const char* YYCURSOR) {
    only_zeros: {
        /*!local:re2c
            [0]         { goto only_zeros;                                                                                                                                                                                                                                              }
            [1-3]       { if constexpr (is_negative) { return lex_trimmed_oct_digits_signed_negative<T, max, min, get_digits, false>(-CHAR_TO_DIGIT('0'), YYCURSOR); } else { return lex_trimmed_oct_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR); }   }
            [3-7]       { if constexpr (is_negative) { return lex_trimmed_oct_digits_signed_negative<T, max, min, get_digits, true >(-CHAR_TO_DIGIT('0'), YYCURSOR); } else { return lex_trimmed_oct_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR); }   }
            *           { RETURN_ONLY_ZEROS()                                                                                                                                                                                                                                           }
        */
    }
}
...
[[gnu::always_inline]] inline ParseNumberResult<T, get_digits> lex_hex_digits_unsigned (const char* YYCURSOR) {
    ...
    /*!local:re2c
        [0]         { goto only_zeros;                                              }
        [1-9]       { value = CHAR_TO_DIGIT('0');      goto not_only_zeros;         }
        [a-f]       { value = CHAR_TO_DIGIT('a') + 10; goto not_only_zeros;         }
        [A-F]       { value = CHAR_TO_DIGIT('A') + 10; goto not_only_zeros;         }
    ...
    */
    ...
    for (size_t i = 1; i < ce::log<16, max> + 2; i++) {
        /*!local:re2c
            [0-9]       { value = (value << 4) + CHAR_TO_DIGIT('0');        continue;   }
            [a-f]       { value = (value << 4) + CHAR_TO_DIGIT('a') + 10;   continue;   }
            [A-F]       { value = (value << 4) + CHAR_TO_DIGIT('A') + 10;   continue;   }
...
[[gnu::always_inline]] inline ParseNumberResult<T, get_digits> lex_hex_digits_signed (const char* YYCURSOR) {
    /*!local:re2c
        [0]         { goto only_zeros;                                                                                                                                                                                                                                                          }
        [1-7]       { if constexpr (is_negative) { return lex_trimmed_hex_digits_signed_negative<T, max, min, get_digits, false>(-CHAR_TO_DIGIT('0')       , YYCURSOR); } else { return lex_trimmed_hex_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0')     , YYCURSOR); }   }
        [8-9]       { if constexpr (is_negative) { return lex_trimmed_hex_digits_signed_negative<T, max, min, get_digits, true >(-CHAR_TO_DIGIT('0')       , YYCURSOR); } else { return lex_trimmed_hex_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0')     , YYCURSOR); }   }
        [a-f]       { if constexpr (is_negative) { return lex_trimmed_hex_digits_signed_negative<T, max, min, get_digits, true >(-(CHAR_TO_DIGIT('a') + 10), YYCURSOR); } else { return lex_trimmed_hex_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('a') + 10, YYCURSOR); }   }
        [A-F]       { if constexpr (is_negative) { return lex_trimmed_hex_digits_signed_negative<T, max, min, get_digits, true >(-(CHAR_TO_DIGIT('A') + 10), YYCURSOR); } else { return lex_trimmed_hex_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('A') + 10, YYCURSOR); }   }
        *           { UNEXPECTED_INPUT("expected at least one hexadecimal digit");                                                                                                                                                                                                              }
    */
    only_zeros: {
...
inline ParseNumberResult<T, get_digits> parse_uint (const char* YYCURSOR) {
    /*!local:re2c
        ...
        [1-9]       { return lex_dec_digits_unsigned<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR);    }
...
inline ParseNumberResult<T, get_digits> parse_uint_skip_white_space (const char* YYCURSOR) {
    /*!local:re2c
        ...
        white_space* [1-9]      { return lex_dec_digits_unsigned<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR);    }
...
inline ParseNumberResult<T, get_digits> parse_uint_skip_any_white_space (const char* YYCURSOR) {
    /*!local:re2c
        ...
        any_white_space* [1-9]      { return lex_dec_digits_unsigned<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR);    }
...
inline ParseNumberResult<T, get_digits> parse_int (const char* YYCURSOR) {
    /*!local:re2c
        ...
        [1-9]       { return lex_dec_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR); }
    ...
    */
    minus_sign: {
        /*!local:re2c
            ...
            [1-9]       { return lex_dec_digits_signed_negative<T, max, min, get_digits>(-CHAR_TO_DIGIT('0'), YYCURSOR);    }
...
inline ParseNumberResult<T, get_digits> parse_int_skip_white_space (const char* YYCURSOR) {
    /*!local:re2c
        ...
        white_space* [1-9]      { return lex_dec_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR); }
...
inline ParseNumberResult<T, get_digits> parse_int_skip_any_white_space (const char* YYCURSOR) {
    /*!local:re2c
        ...
        any_white_space* [1-9]      { return lex_dec_digits_signed_positive<T, max, min, get_digits>(CHAR_TO_DIGIT('0'), YYCURSOR); }
